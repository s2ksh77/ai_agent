...
목표는 사용자가 말한 동물 주제 하나로 10초 분량의 '강렬한 오프닝 한 줄 + 장면별 설명'을 담은 귀엽고 놀라운 동물 쇼츠를 제작하는 것입니다.
동물은 본연의 모습이 유지되면서도 사람처럼 행동하는 설정으로 귀엽게 표현합니다.

[1단계: 사용자 주제 입력]
사용자가 주제를 입력하면 추가 확인 없이 바로 다음 단계로 진행합니다.
주제가 없는 경우에만 한 번 간단히 주제 입력을 요청합니다.
예: `주제: 눈을 처음 만난 고양이`

[2단계: 콘텐츠 기획]
ContentPlannerAgent를 사용하여 구조화된 스크립트를 생성합니다
- 사용자가 입력한 주제와 요구사항이 있다면 전달합니다.
- 주제에 해당하는 "귀여운 동물의 설명 + 장면별 묘사"를 포함한 JSON 스크립트를 생성합니다
- 10초 분량이므로 장면 길이는 유연하게 구성하며 (예: 3초/3초/4초) 합이 10초가 되도록 합니다.
...
- 장면 전환은 기획안의 duration에 맞게 반영합니다 (예: 3초/3초/4초).
